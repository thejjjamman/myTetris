...
public class playing extends state {
	...
	private int timeInterval = 20;// 50;//50 is good to start
	...
	protected playing(GameStateManager gsmIn) {
		super(gsmIn);
		backGround = new Texture("blackBackground.png");

		stopedBlocks = new block[12][26];
		movingBlocks = new block[12][26];// so yeah my arrays are backwords with
											// the gdx libary calling bot left
											// 0,0 and me calling top left 0.0
		futureBlock = new block[8][7];
		...
		for (int x = 0; x < stopedBlocks.length; x++) {
			for (int y = 0; y < stopedBlocks[x].length; y++) {
				if (y == 0 || y == stopedBlocks[x].length - 1 || x == 0 || x == stopedBlocks.length - 1) {
					stopedBlocks[x][y] = new block(x * 25, y * 25, GRAY, false);
				}
			}
		}
	}

	private void fillFutreBlocks() {

		for (int x = 0; x < futureBlock.length; x++) {
			for (int y = 0; y < futureBlock[x].length; y++) {
				if (y == 0 || y == futureBlock[x].length - 1 || x == 0 || x == futureBlock.length - 1) {
					futureBlock[x][y] = new block((x * 25) + futureXPix, (y * 25) + futureXPix, GRAY, false);
				}
			}
		}
	}
	...
	private void addFutureShape(int code) {
		for (int x = 1; x < futureBlock.length - 1; x++) {
			for (int y = 1; y < futureBlock[x].length - 1; y++) {
				if (futureBlock[x][y] != null) {
					futureBlock[x][y] = null;
				}
			}
		}
		...
		if (code == 1) {// square
			futureBlock[x][y] = new block(x * 25, y * 25, LIGHTBLUE, false);
			futureBlock[x + 1][y] = new block((x + 1) * 25, y * 25, LIGHTBLUE, false);

			futureBlock[x][y - 1] = new block(x * 25, (y - 1) * 25, LIGHTBLUE, false);
			futureBlock[x + 1][y - 1] = new block((x + 1) * 25, (y - 1) * 25, LIGHTBLUE, false);
		}

		else if (code == 2) {// pir
			futureBlock[x - 1][y] = new block((x - 1) * 25, y * 25, YELOW, false);
			futureBlock[x][y] = new block(x * 25, y * 25, YELOW, false);
			futureBlock[x + 1][y] = new block((x + 1) * 25, y * 25, YELOW, false);
			futureBlock[x][y - 1] = new block(x * 25, (y - 1) * 25, YELOW, true);

		} else if (code == 3) {// line
			futureBlock[x - 1][y] = new block((x - 1) * 25, y * 25, RED, false);
			futureBlock[x][y] = new block(x * 25, y * 25, RED, false);
			futureBlock[x + 1][y] = new block((x + 1) * 25, y * 25, RED, false);
			futureBlock[x + 2][y] = new block((x + 2) * 25, y * 25, RED, false);
		}

		else if (code == 4) {// left block L
			futureBlock[x - 1][y] = new block((x - 1) * 25, y * 25, BLUE, false);
			futureBlock[x][y] = new block(x * 25, y * 25, BLUE, false);
			futureBlock[x + 1][y] = new block((x + 1) * 25, y * 25, BLUE, false);
			futureBlock[x - 1][y - 1] = new block((x - 1) * 25, (y - 1) * 25, BLUE, false);

		} else if (code == 5) {// right block L
			futureBlock[x - 1][y] = new block((x - 1) * 25, y * 25, ORANGE, false);
			futureBlock[x][y] = new block(x * 25, y * 25, ORANGE, false);
			futureBlock[x + 1][y] = new block((x + 1) * 25, y * 25, ORANGE, false);
			futureBlock[x + 1][y - 1] = new block((x + 1) * 25, (y - 1) * 25, ORANGE, false);
		} else if (code == 6) {// -_ shape

			futureBlock[x][y] = new block(x * 25, y * 25, PURPLE, false);
			futureBlock[x + 1][y] = new block((x + 1) * 25, y * 25, PURPLE, false);
			futureBlock[x + 1][y - 1] = new block((x + 1) * 25, (y - 1) * 25, PURPLE, false);
			futureBlock[x + 2][y - 1] = new block((x + 2) * 25, (y - 1) * 25, PURPLE, false);
		}

		else if (code == 7) {// _- shape
			futureBlock[x + 1][y] = new block((x + 1) * 25, y * 25, GREEN, false);
			futureBlock[x + 2][y] = new block((x + 2) * 25, y * 25, GREEN, false);
			futureBlock[x + 1][y - 1] = new block((x + 1) * 25, (y - 1) * 25, GREEN, false);
			futureBlock[x][y - 1] = new block((x + 2) * 25, (y - 1) * 25, GREEN, false);
		} else if (code == 8) {// mike
			futureBlock[x][y] = new block((x + 1) * 25, y * 25, MIKE, true);
	...
	public void update(float dt) {
		...
		timeCountInput++;
		handleInput();
		//if (timeCountInput > timeInterval / 2) {
			//handleHoldDownInput();
		//	timeCountInput = 0;} 
	...
	private void triggerNextShape() {
		...
		addFutureShape(FutureShapeCode);
		...
		for (int y = 1; y < stopedBlocks[1].length - 1; y++) {
			...
			if (rowLength >= 10) {
				for (int x = 1; x < stopedBlocks.length - 1; x++) {
					...
					// mow get all stoped blocks to move exept top and bot row
					for (int yy = y; yy < stopedBlocks[x].length - 1; yy++) {

						if (stopedBlocks[x][yy] != null) {// if block
															// exisits

							stopedBlocks[x][yy] = stopedBlocks[x][yy - 1];

						}
						// special ability or all same colour row.
						// moveBlockASFarDownAsPosible(x,yy);
			...
			for (int y = 0; y < futureBlock[x].length; y++) {
				if (futureBlock[x][y] != null) {
					sb.draw(futureBlock[x][y].getImg(), futureBlock[x][y].getPosition().x,
							futureBlock[x][y].getPosition().y-25);
	...
	public void render(SpriteBatch sb) {
		...
		yourBitmapFontName.draw(sb, "SCORE : "+(score*1), 325, 450); 
